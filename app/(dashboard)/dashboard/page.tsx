  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Orders",       value: loading ? "-" : orders.length.toString(),                 sub: "All time",                   icon: ShoppingBag,   color: "text-accent-primary",   bg: "bg-accent-primary/10", spotColor: "#2563eb" },
          { label: "Active Services",    value: loading ? "-" : activeOrdersCount.toString(),             sub: "Research & academic work",   icon: FileText,      color: "text-accent-warning",   bg: "bg-accent-warning/10", spotColor: "#f59e0b" },
          { label: "Store Orders",       value: loading ? "-" : hwPendingCount.toString(),                sub: "Electronics pending",        icon: Package,       color: "text-blue-600",         bg: "bg-blue-50",           spotColor: "#3b82f6" },
          { label: "Wallet Balance",     value: "₹0",                                                    sub: "Available credits",          icon: Wallet,        color: "text-accent-success",   bg: "bg-accent-success/10", spotColor: "#10b981" },
        ].map((stat) => (
          <div 
            key={stat.label}
            onMouseMove={handleMouseMove}
            className="relative p-[1.5px] rounded-2xl overflow-hidden group bg-slate-200/40 hover:bg-transparent transition-colors duration-500 shadow-sm"
            style={{
              // @ts-ignore
              "--spotlight-color": stat.spotColor
            }}
          >
            {/* Bold Colorful Spotlight Border */}
            <div 
              className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-0"
              style={{
                background: `radial-gradient(130px circle at var(--mouse-x, 0px) var(--mouse-y, 0px), var(--spotlight-color) 0%, #6366f1 40%, #a855f7 70%, #ec4899 100%)`
              }}
            />
            
            {/* Inner Content */}
            <div className="relative z-10 bg-bg-card rounded-[14px] p-5 flex flex-col gap-3 h-full">
      ...
      >
        ...
        <div 
          className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-0"
          style={{
            background: `radial-gradient(220px circle at var(--mouse-x, 0px) var(--mouse-y, 0px), var(--spotlight-color) 0%, #6366f1 40%, #a855f7 70%, #ec4899 100%)`
          }}
        />

        <div className="relative z-10 bg-bg-card rounded-[14px] overflow-hidden">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Track Orders",     href: "/dashboard/orders",  icon: ShoppingBag, color: "text-accent-primary bg-accent-primary/10", spotColor: "#2563eb" },
            { label: "New Request",      href: "/services",           icon: Plus,        color: "text-accent-success bg-accent-success/10", spotColor: "#10b981" },
            { label: "Support Chat",     href: "/dashboard/messages", icon: MessageSquare, color: "text-accent-secondary bg-accent-secondary/10", spotColor: "#0f172a" },
            { label: "AI Patent Drafter", href: "/patent-drafter",   icon: FileText,    color: "text-accent-warning bg-accent-warning/10", spotColor: "#f59e0b" },
          ].map((qa) => (
            <Link key={qa.label} href={qa.href} className="block group">
              <div 
                onMouseMove={handleMouseMove}
                className="relative p-[1.5px] rounded-2xl overflow-hidden bg-slate-200/40 hover:bg-transparent transition-colors duration-500 shadow-sm h-full"
                style={{
                  // @ts-ignore
                  "--spotlight-color": qa.spotColor
                }}
              >
                {/* Bold Colorful Spotlight Border */}
                <div 
                  className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-0"
                  style={{
                    background: `radial-gradient(100px circle at var(--mouse-x, 0px) var(--mouse-y, 0px), var(--spotlight-color) 0%, #6366f1 40%, #a855f7 70%, #ec4899 100%)`
                  }}
                />

                {/* Inner Content */}
                <div className="relative z-10 bg-bg-card rounded-[14px] p-5 flex flex-col items-center text-center gap-3 hover:-translate-y-0.5 transition-all duration-300 h-full justify-center">
      ...
      >
        ...
        <div 
          className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-0"
          style={{
            background: `radial-gradient(200px circle at var(--mouse-x, 0px) var(--mouse-y, 0px), var(--spotlight-color) 0%, #6366f1 40%, #a855f7 70%, #ec4899 100%)`
          }}
        />

        <div className="relative z-10 bg-gradient-to-r from-accent-primary/10 to-accent-secondary/10 rounded-[14px] p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <Link href="/dashboard/profile" className="flex-shrink-0 w-full sm:w-auto">
