...
import { getMessages, sendMessage as sendServerMessage, markAsRead } from "@/app/actions/messages";
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    const fetchMsgs = async () => {
      ...
      // Clear unread messages count on server
  ...
  return (
    <div className="space-y-6 w-full h-[calc(100vh-140px)] flex flex-col">
      {/* Header */}
      <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm relative overflow-hidden shrink-0 flex items-center justify-between">
        <div>
          <h1 className="font-heading font-black text-2xl md:text-3xl text-slate-900 tracking-tight flex items-center gap-2">
            <Sparkles className="w-6 h-6 text-indigo-655 animate-pulse" />
            Messages Corridor
          </h1>
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center mt-1">
                      <div className="flex items-center gap-2">
                        ...
                        {conv.unreadCount > 0 && (
                          <span className="bg-red-500 text-white text-[8px] font-black w-4.5 h-4.5 rounded-full flex items-center justify-center animate-pulse">
                            {conv.unreadCount}
                          </span>
                        )}
                      </div>
          ...
          {activeConv ? (
            <>
              ...
              <div className="p-4 md:px-6 border-b border-slate-150 bg-white flex items-center justify-between shadow-sm relative z-10 shrink-0">
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-[9px] font-mono text-slate-455 uppercase tracking-wider">Tunnel: {activeConv.orderNumber}</span>
                      ...
                      <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                        <div className="max-w-[80%] space-y-1">
                          ...
                          <div className={`flex items-center gap-1.5 ${isMe ? "justify-end" : "justify-start"}`}>
                            {!msg.approvedByAdmin && msg.senderRole !== "ADMIN" && (
                              <span className="text-[8px] font-black text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-100 flex items-center gap-0.5">
                            ...
                            {isMe && msg.approvedByAdmin && <CheckCircle className="w-3.5 h-3.5 text-indigo-600" />}
                          </div>
                        </div>
                      </div>
                    );
              ...
              <div className="px-4 py-2.5 bg-white border-t border-slate-100 flex items-center gap-2 overflow-x-auto select-none shrink-0 scrollbar-none">
                ...
                {QUICK_TEMPLATES.map((tpl, i) => (
                  <button
                    ...
                    className="px-3 py-1 bg-slate-50 border border-slate-200 hover:border-indigo-400 rounded-xl text-[9px] font-black uppercase tracking-wider text-slate-655 hover:bg-indigo-50/50 hover:text-indigo-650 transition-colors whitespace-nowrap"
                  >
                    {tpl}
                  </button>
                ))}
              ...
              <div className="p-4 border-t border-slate-150 bg-white shadow-lg relative z-10 shrink-0">
                ...
                <p className="text-[8px] font-bold text-slate-450 mt-2 text-center uppercase tracking-wider leading-relaxed">
                  Sharing phone numbers, personal emails, or social tags violates NDA security protocols. Messages are routed via Admin approval.
                </p>
              </div>
            </>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-slate-50/20">
              <MessageSquare className="w-12 h-12 text-slate-350 animate-pulse mb-3" />
