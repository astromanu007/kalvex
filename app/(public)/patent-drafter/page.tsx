              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Product Name</label>
                    <input value={productName} onChange={e => setProductName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 ring-blue-500/20 outline-none font-bold text-slate-700" placeholder="e.g. AI-Based Detection System" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Filing Date</label>
                    <input value={dated} onChange={e => setDated(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 ring-blue-500/20 outline-none font-bold text-slate-700" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 md:grid-cols-7 gap-2">
                  {PATENT_VIEWS.map(v => (
                    <label key={v.id} className="cursor-pointer">
                      <div className={`relative aspect-square rounded-xl border-2 border-dashed flex flex-col items-center justify-center p-1 transition-all ${views[v.id] ? "border-green-500 bg-green-50" : "border-slate-200 hover:border-blue-500 bg-slate-50"}`}>
                        {views[v.id] ? (
                          <>
                          ...
                          <div className="text-center">
                            <FileImage className="w-3 h-3 mx-auto text-slate-400 mb-0.5" />
                            <span className="text-[6px] font-bold text-slate-400 uppercase leading-none">{v.id}</span>
                          </div>
                        )}
                      </div>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  ...
                  <Reorder.Group axis="y" values={authors} onReorder={setAuthors} className="space-y-2">
                    {authors.map((a) => (
                      <Reorder.Item key={a.id} value={a} className="flex gap-2 items-center bg-slate-50 p-3 rounded-2xl border border-slate-200 cursor-default group">
                        <GripVertical className="w-4 h-4 text-slate-300 group-hover:text-slate-500 cursor-grab active:cursor-grabbing shrink-0" />
                        <input value={a.name} onChange={e => setAuthors(p => p.map(x => x.id === a.id ? {...x, name: e.target.value} : x))} placeholder="Full Name" className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs uppercase font-bold text-slate-700 focus:border-blue-300 outline-none transition-colors" />
                        <label className="cursor-pointer bg-white border border-slate-200 rounded-lg px-3 py-2 text-[9px] font-bold hover:bg-slate-100 transition-colors shrink-0 whitespace-nowrap" onClick={e => e.stopPropagation()}>
                          {a.signature ? "✓ Signed" : "Upload Sign"}
                          <input type="file" className="hidden" onChange={e => handleUpload(a.id, e, 'sig', a.id)} />
                        </label>
                        {a.signature && (
                          <div className="relative group shrink-0">
                            <img src={a.signature} className="w-8 h-8 rounded border bg-white object-contain" alt="s" />
                            <button 
                        ...
                        <button onClick={() => removeAuthor(a.id)} className="p-2 text-slate-300 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </Reorder.Item>
                    ))}
                  </Reorder.Group>
                </div>
