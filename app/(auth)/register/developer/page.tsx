        ...
        <AnimatePresence mode="wait">
          {step < 4 ? (
            ...
            >
              {step === 1 && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" required placeholder="Rahul Sharma" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-6 py-4 text-slate-900 font-mono focus:border-blue-600 focus:bg-white outline-none transition-all placeholder:text-slate-300 shadow-sm" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Email Address</label>
                    <input type="email" required placeholder="engineer@example.com" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-6 py-4 text-slate-900 font-mono focus:border-blue-600 focus:bg-white outline-none transition-all placeholder:text-slate-300 shadow-sm" />
                  </div>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Github Profile</label>
            ...
            <div className="text-center py-10 space-y-8 animate-in zoom-in duration-500">
              ...
              <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter italic">Registration Successful</h3>
              <p className="text-slate-500 font-bold font-mono">Verification email sent to <span className="text-blue-600">{formData.email}</span></p>
              <Button onClick={() => router.push("/dashboard")} className="w-full h-16 bg-slate-900 rounded-2xl text-white font-black uppercase tracking-[0.3em] text-xs hover:bg-blue-600 transition-all duration-700 shadow-2xl">Go to Dashboard</Button>
            </div>
          )}
        </AnimatePresence>
      </div>
